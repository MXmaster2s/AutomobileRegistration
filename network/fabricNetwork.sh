...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/partsFactory.automobile-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/PARTSFACTORY_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/carFactory.automobile-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/CARFACTORY_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/dealer.automobile-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/DEALER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  chmod 777 /home/rohit/workspace/automobile-registration/network/bin/configtxgen
  which configtxgen
  if [ "$?" -ne 0 ]; then
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID automobile-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for partsFactoryMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/partsFactoryMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg partsFactoryMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for partsFactory..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for carFactoryMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/carFactoryMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg carFactoryMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for carFactory..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for dealerMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/dealerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg dealerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for dealer..."
    exit 1
  fi
}
